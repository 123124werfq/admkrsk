...
class MultiUploadBehavior extends Behavior
{
	protected $fk;
	...
	*/
	...
	public function beforeValidate($event)
	{
		...
		foreach ($this->relations as $relation_name => $settings)
		{
			...
			if (!empty($POSTs))
			{
				$classModel = "common\models\\".$settings['model'];
	...
	public function afterSave($event)
	{
		...
		foreach ($this->relations as $relation_name => $settings)
		{
			...
			foreach ($this->records[$relation_name] as $key=>$mediaModel)
			{
				...
				if ($mediaModel->isNewRecord)
				{
					...
					if ($mediaModel->save())
					{
						...
						if (!empty($settings['jtable']))
						{
							$this->owner->link($relation_name,$mediaModel,(!empty($settings['jtable_insert']))?$settings['jtable_insert']:[]);
						}
			...
			if (!empty($delete_ids))
			{
				...
				Yii::$app->db->createCommand()->delete($mediaModel->tableName(), [$media_pk=>$delete_ids])->execute();
			}
			...
			if (!empty($settings['fk_cover']))
			{
				...
				Yii::$app->db->createCommand()->update($Owner->tableName(),[$settings['fk_cover']=>$fk_cover],[$pk_field=>$pk_value])->execute();
