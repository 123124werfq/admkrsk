...
use common\components\softdelete\SoftDelete;
...
class NestedSetsBehavior extends Behavior
{
    ...
    public function events()
    {
        return [
            ...
            SoftDelete::EVENT_BEFORE_RESTORE => 'beforeRestore',
            SoftDelete::EVENT_AFTER_RESTORE => 'afterRestore',
        ];
    }
     ...
     * @return \yii\db\ActiveQuery
     */
    public function parentsDeleted($depth = null)
    {
        $condition = [
            'and',
            ['<', $this->leftAttribute, $this->owner->getAttribute($this->leftAttribute)],
        ...
        return $this->owner->findDeleted()->andWhere($condition)->addOrderBy([$this->leftAttribute => SORT_ASC]);
    }
     ...
     * @return \yii\db\ActiveQuery
     */
    public function parentsWithDeleted($depth = null)
    {
        $condition = [
            'and',
            ['<', $this->leftAttribute, $this->owner->getAttribute($this->leftAttribute)],
        ...
        return $this->owner->findWithDeleted()->andWhere($condition)->addOrderBy([$this->leftAttribute => SORT_ASC]);
    }
     ...
     * @return \yii\db\ActiveQuery
     */
    public function childrenDeleted($depth = null)
    {
        $condition = [
            'and',
            ['>', $this->leftAttribute, $this->owner->getAttribute($this->leftAttribute)],
     ...
     * @return \yii\db\ActiveQuery
     */
    public function childrenWithDeleted($depth = null)
    {
        $condition = [
            'and',
            ['>', $this->leftAttribute, $this->owner->getAttribute($this->leftAttribute)],
    ...
    public function afterDelete()
    {
        ...
        foreach ($this->owner->children(1)->each() as $child) {
            $child->delete();
        }
    ...
    {
        foreach ($this->owner->parentsDeleted(1)->each() as $parent) {
            $parent->restore();
        }
    ...
    {
        // раскомменить, если нужно восстановление вместе с дочерними
        // foreach ($this->owner->childrenDeleted(1)->each() as $child) {
        //     $child->restore();
        // }
