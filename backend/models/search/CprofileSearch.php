        ...
        */
        ...
        if(isset($contests[$this->id_contest]))
            $sql = "SELECT cp.*, id_record FROM cst_profile cp 
                ...
                WHERE ff.alias = '{$contests[$this->id_contest]['participant_form']}' and cr.created_by is not null and cr.deleted_at is null and cr.is_archive != 1";
        else
        {
            //$sql = "SELECT * FROM cst_profile cp";
            $sql = "SELECT cp.*, id_record FROM cst_profile cp 
                left join db_collection_record cr on cr.id_record = cp.id_record_anketa 
                WHERE cr.created_by is not null and cr.deleted_at is null and cr.is_archive != 1";
        }
